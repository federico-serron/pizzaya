    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50">
          <div className="absolute inset-0 bg-warm-950/60 backdrop-blur-sm" onClick={() => setOpen(false)} />

          {/* Drawer */}
          <div className="absolute right-0 top-0 h-full w-full max-w-sm bg-[var(--bg-secondary)] border-l border-[var(--border)] shadow-2xl animate-fade-up overflow-hidden flex flex-col">
            {/* Header */}
                  ...
                  {items.map((item) => (
                    <div key={item.product_id} className="flex items-center gap-3 p-3 rounded-xl bg-[var(--bg-tertiary)] border border-[var(--border)]">
                      ...
                      <div className="flex items-center gap-1">
                        <button onClick={() => updateItem(item.product_id, item.quantity - 1)} className="w-7 h-7 flex items-center justify-center rounded-lg bg-[var(--bg-secondary)] hover:bg-[var(--border)] text-[var(--text-secondary)] transition-colors">
                          <Minus size={11} />
                        </button>
                      ...
                      <button onClick={() => removeItem(item.product_id)} className="p-1.5 rounded-lg hover:bg-rose-100 dark:hover:bg-rose-900/30 text-[var(--text-muted)] hover:text-rose-600 transition-colors">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  ))}
            ...
            {items.length > 0 && (
              <div className="border-t border-[var(--border)] p-5 space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={emptyCart} className="btn-secondary !py-2.5 text-xs flex-1">
                    Vaciar
                  </button>
                  <Link
                    to="/checkout"
                    onClick={() => setOpen(false)}
                    className="btn-primary !py-2.5 text-sm flex-1 text-center"
                  >
                    Ir a pagar
                  </Link>
                </div>
